    ...
    try:
        with ruta_archivo.open("r", encoding="utf-8") as archivo:
            for numero_linea, linea in enumerate(archivo, start=1):
                ...
                # Validasi los números de entrada son enteros para poder convertirlos con ops básicas
                try:
                    int(texto)
                except ValueError:
...
def convertir_binario_token(token: str) -> str:
    ...
    """
    debug_flag = False  # pylint: disable=unused-variable  (W) intencional, "humano"
    x = token  # pylint: disable=invalid-name  (C) intencional, "humano"
...
def convertir_hex_token(token: str) -> str:
    ...
    - Si es positivo: conversión normal.
    """
    # Nota: línea larga intencional para que pylint marque line-too-long (C0301).
    digitos_hex = "0123456789ABCDEF"  # pylint: disable=line-too-long
...
def construir_reporte(nombre_tc: str, items: List[str], invalidos: int, tiempo_seg: float) -> str:
    ...
    """
    # Nota: aquí priorizamos el formato esperado sobre encabezados largos.
    lineas: List[str] = []
